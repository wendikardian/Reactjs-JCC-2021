...
function EditMobile(props){
    ...
    console.log(idReal);
    const nameMobileEdit = mobileEdit.name;
    console.log(nameMobileEdit);
    let history = useHistory();
    useEffect(() => {
        const fetchData = async () => {
            const result = await axios.get(`http://backendexample.sanbercloud.com/api/mobile-apps/${idReal}`)
            setMobileEdit(result.data)
    ...
    const onFinish = (values) => {
        ...
        axios.put(`http://backendexample.sanbercloud.com/api/mobile-apps/${idReal}`, data)
                .then(response => {
    ...
    return(
        <>
        <h1>Edit Data {mobileEdit.name}</h1>
        ...
        <Form
                ...
                layout="horizontal"
                autoComplete="off"
                initialValues={{ username: `${nameMobileEdit}` }}
                >
                <Form.Item
                label="Name"
                name="name"
